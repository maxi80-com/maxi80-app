...
- **[RESOLVED — spike done 2026-07-16] The service MUST be a raw `.kt` file in `Sources/Maxi80Services/Skip/`, not transpiled Swift.** A throwaway `MediaSessionService` subclass in Swift `#if SKIP` transpiled and passed `skip android build`, but the generated Kotlin failed to compile: Skip emits `class Foo: Bar` (no parentheses) for superclasses, whereas Kotlin requires `MediaSessionService()` (constructor invocation) for abstract Android framework superclasses → `error: This type has a constructor, so it must be initialized here`. `BroadcastReceiver`/`MediaSession.Callback` subclasses in Swift still work; only a framework-lifecycle `Service` is affected. Decision: author the service as `.kt` (package `maxi80.services`), calling into the transpiled `SharedAudioPlayer`. The confirmed transpiled package for `Maxi80Services` types is `maxi80.services`.

- **[Evaluated — not applicable] SkipAV (`skip.dev/docs/modules/skip-av/`).** SkipAV wraps a subset of AVFoundation (AVPlayer/AVAudioPlayer/AVAudioRecorder/VideoPlayer) over media3 ExoPlayer on Android, so it superficially matches this app's player abstraction. It is **not usable here**: it wraps only the *player*, providing **no `MediaSession`/`MediaSessionService`, no media notification / lock-screen / remote controls, and no Android Auto** — exactly the session/service layer this whole design builds. Streaming + ICY metadata are also undocumented (AVPlayer support rated "Medium", "anything not listed is likely not supported"). The app's direct-to-`AVPlayer`/-media3 approach via `AudioStreamPlayer` is the correct choice because it retains full access to media3's session APIs that Android Auto requires. SkipAV could only ever simplify the plain play/pause/volume path, not the session/Auto layer — not worth reworking a working streaming path for.
