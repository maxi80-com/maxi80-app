import Foundation
    ...
    extension AudioStreamPlayer {
      ...
      private func macObservePlayerStatus() {
        macStatusObservation = macPlayer?.observe(\.currentItem?.status, options: [.new]) {
        ...
        macTimeControlObservation = macPlayer?.observe(\.timeControlStatus, options: [.new]) {
          @Sendable [weak self] player, _ in
    ...
    extension AudioStreamPlayer {
      ...
      var macPlayer: AVPlayer? {
        ...
        set {
          objc_setAssociatedObject(
            self, &macPlayerKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
      }

      var macMetadataDelegate: MacMetadataOutputDelegate? {
        get {
          objc_getAssociatedObject(self, &macMetadataDelegateKey) as? MacMetadataOutputDelegate
        }
        set {
          objc_setAssociatedObject(
            self, &macMetadataDelegateKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
      }
      ...
      var macStatusObservation: NSKeyValueObservation? {
        ...
        set {
          objc_setAssociatedObject(
            self, &macStatusObservationKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
      }

      var macTimeControlObservation: NSKeyValueObservation? {
        get {
          objc_getAssociatedObject(self, &macTimeControlObservationKey) as? NSKeyValueObservation
        }
        set {
          objc_setAssociatedObject(
            self, &macTimeControlObservationKey, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
      }
    }
    ...
    final class MacMetadataOutputDelegate: NSObject, AVPlayerItemMetadataOutputPushDelegate,
      @unchecked Sendable
    {
