...
struct ReconnectionPropertyTests {
  ...
  func backoffDelaySequence() {
    ...
    property("delay(n) == 2^n seconds in nanoseconds for n in 1...3")
      <- forAll(Gen<Int>.fromElements(in: 1...3)) { attempt in
  ...
  func reconnectionStopsAfterMaxAttempts() {
    property("for failure counts 1-5, only first 3 have valid backoff delays")
      <- forAll(Gen<Int>.fromElements(in: 1...5)) { failureCount in
        let manager = ReconnectionManager()
        let maxAttempts = 3
