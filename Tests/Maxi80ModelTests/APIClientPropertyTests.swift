  ...
  public static var arbitrary: Gen<APIEndpoint> {
    ...
    let artworkGen = Gen<APIEndpoint>.compose { composer in
      let artist = composer.generate(
        using: String.arbitrary.map {
          $0.trimmingCharacters(in: .whitespaces)
        })
      let title = composer.generate(
        using: String.arbitrary.map {
          $0.trimmingCharacters(in: .whitespaces)
        })
...
struct APIClientPropertyTests {
  ...
  func authTokenAlwaysIncluded() {
    property("Authorization header equals configured token for all endpoints")
      <- forAll { (endpoint: APIEndpoint, apiKey: String) in
  ...
  func allEndpointTypesIncludeAuthHeader() {
    property("station, artwork, and history requests all have Authorization header")
      <- forAll { (apiKey: String) in
        guard !apiKey.isEmpty else { return true }
